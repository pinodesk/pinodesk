...
public class ProductImportController extends CommonContentPaneController {
    ...
    void onActionBtnDownload(ActionEvent event) {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            displayInfo(String.format(translate(MessageCode.SUCCESS_DOWNLOAD_TEMPLATE), location));
        }));
    ...
    protected void initContentPaneControlActions() {
        ...
        fileChooser.getExtensionFilters()
                .addAll(new FileChooser.ExtensionFilter(translate(CommonLabel.LBL_XLSX_FILES), "*.xlsx"));
    ...
    private void createSheetProduct(XSSFWorkbook workbook) {
        XSSFSheet sheet = workbook.createSheet(translate(CommonLabel.LBL_PRODUCT));
        XSSFRow row = sheet.createRow(0);
        for (int i = 0; i < PRODUCT_SHEET_COLUMN_LABELS.length; i++) {
            row.createCell(i).setCellValue(translate(PRODUCT_SHEET_COLUMN_LABELS[i]));
    ...
    private void createSheetProductCategory(XSSFWorkbook workbook) {
        XSSFSheet sheet = workbook.createSheet(translate(CommonLabel.LBL_PRODUCT_CATEGORY));
        XSSFRow row = sheet.createRow(0);
        for (int i = 0; i < PRODUCT_CATEGORY_SHEET_COLUMN_LABELS.length; i++) {
            row.createCell(i).setCellValue(translate(PRODUCT_CATEGORY_SHEET_COLUMN_LABELS[i]));
        }
        int rowNum = 1;
    ...
    private void createSheetUnit(XSSFWorkbook workbook) {
        XSSFSheet sheet = workbook.createSheet(translate(CommonLabel.LBL_UNIT));
        XSSFRow row = sheet.createRow(0);
        for (int i = 0; i < UNIT_SHEET_COLUMN_LABELS.length; i++) {
            row.createCell(i).setCellValue(translate(UNIT_SHEET_COLUMN_LABELS[i]));
        }
        int rowNum = 1;
