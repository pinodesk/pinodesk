...
public class ProductPackageAddController extends CommonDataSaveController {
    ...
    void onActionBtnAddProduct(ActionEvent event) {
        ...
        PackageProductVM pp = new PackageProductVM();
        pp.setProduct(selectedProduct);
        pp.setQuantity(toIntegerOrDefault(tfQuantity.getText(), 1));
    ...
    protected void initDataSaveControlActions() {
        ...
        TableViewUtils.setColumnValue(colName, pp -> pp.getProduct().getName());
        TableViewUtils.setColumnValue(colUnit, pp -> pp.getProduct().getUnitLabel());
        ...
        TableViewUtils.initTableColumn(
                ...
                new NumberCellFactory<>(locale),
                pp -> pp.getProduct().getGeneralSellingPrice(),
                StyleConstants.ALIGN_RIGHT);
        TableViewUtils.initTableColumn(
                colPrescriptionSellingPrice,
                new NumberCellFactory<>(locale),
                pp -> pp.getProduct().getPrescriptionSellingPrice(),
                StyleConstants.ALIGN_RIGHT);
    ...
    private int getProductIndexInTable(ProductVM product, TableView<PackageProductVM> table) {
        Predicate<PackageProductVM> productExists = item -> item.getProduct().getId().equals(product.getId());
    ...
    private void calculatePrice() {
        ...
        for (PackageProductVM pp : tblProducts.getItems()) {
            ProductVM p = pp.getProduct();
            Integer qty = pp.getQuantity();
            if (p.getGeneralSellingPrice() != null) {
                BigDecimal total = p.getGeneralSellingPrice().multiply(BigDecimal.valueOf(qty));
                generalSellPrice = generalSellPrice == null ? total : generalSellPrice.add(total);
            }
            if (p.getPrescriptionSellingPrice() != null) {
                BigDecimal total = p.getPrescriptionSellingPrice().multiply(BigDecimal.valueOf(qty));
