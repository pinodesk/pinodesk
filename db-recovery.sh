...
mkdir -p ~/.pinus/db/temp

echo "Move corrupted db to temp folder"
mv pinusdb.mv.db pinusdb.trace.db ~/.pinus/db/temp

echo "Execute recovery SQL script"
java -cp ~/.m2/repository/com/h2database/h2/2.1.212/h2-2.1.212.jar org.h2.tools.RunScript \
-url "jdbc:h2:~/.pinus/db/pinusdb;MODE=MySQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_ON_EXIT=FALSE" \
-script pinusdb.h2.sql
